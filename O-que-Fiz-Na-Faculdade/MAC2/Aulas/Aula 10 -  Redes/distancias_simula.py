...
# Redes
# escolha a rede e descomente a linha correspondente
from rede_matriz import Rede # Killed para rede grande
# from rede_numpy import Rede  # MemoryError para rede grande
# from rede_list import Rede   # aguenta rede maior
# from rede_dict import Rede   # aguenta rede maior

# imprime mensagem de um passo a passo
VERBOSE = True

def main():
    origem = int(input("Digite a cidade origem: "))
    nome_arquivo = input("Digite o nome do arquivo txt com a rede: ")
    
    print("criando rede")
    rede = leia_rede(nome_arquivo)
    print("rede criada")
    
    if VERBOSE: print(rede)
    
    # calcule as distancias
    d = distancias(rede, origem)
...
def distancias(rede, c):
    ...
    while not q.isEmpty():
        ...
        if VERBOSE:
            print("cidade que sera examinada: ", i)
            print("d =", d)
            print("q =", q)
            pause()
            
        # examine as cidades vizinhas da cidade i
        for j in rede.vizinhas(i):
            if d[j] > d[i]+1:
                d[j] = d[i] + 1
                q.enqueue(j)
                
        if VERBOSE:
            print("depois de examinar: ", i)
            print("d =", d)
            print("q =", q)
            pause()
