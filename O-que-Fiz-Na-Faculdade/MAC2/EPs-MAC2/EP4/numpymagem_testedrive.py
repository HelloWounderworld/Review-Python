...
'''
Nota
Revisado em quinta, 5 set 2019, 11:17 por Atribuição automática de nota
grade: 8,0 / 10,0
...
crop(): iniciando avaliação dos testes (vale 0.75 ponto(s))
crop() passou em todos os testes :-)
crop(): avaliação encerrada... (nota 0.75)

__add__(): iniciando avaliação dos testes (vale 0.75 ponto(s))
__add__() passou em todos os testes :-)
__add__(): avaliação encerrada... (nota 0.75)

__mul__(): iniciando avaliação dos testes (vale 0.75 ponto(s))
__mul__() passou em todos os testes :-)
__mul__(): avaliação encerrada... (nota 0.75)

paste(): iniciando avaliação dos testes (vale 0.75 ponto(s))
paste() passou em todos os testes :-)
paste(): avaliação encerrada... (nota 0.75)

pinte_disco(): iniciando avaliação dos testes (vale 0.75 ponto(s))
pinte_disco() passou em todos os testes :-)
pinte_disco(): avaliação encerrada... (nota 0.75)

pinte_retangulo(): iniciando avaliação dos testes (vale 0.75 ponto(s))
pinte_retangulo() passou em todos os testes :-)
pinte_retangulo(): avaliação encerrada... (nota 0.75)

transpoe(): iniciando avaliação dos testes (vale 1 ponto(s))
NumPymagem[[ 1 -1]
[ 1 1]].transpoe() tem erro na posição [0][1].
NumPymagem[[1 0 0]
[0 1 1]
[1 1 1]].transpoe() tem erro na posição [0][2].
NumPymagem[[ 0 0 0]
[ 0 -1 0]].transpoe() tem linha com tamanho errado.
NumPymagem[[ 1. 1. 1. 1. 1. 1. ]
[-1. -1. -1. 0.5 0.5 0.5]].transpoe() tem erro na posição [0][1].
NumPymagem[[1 0]
[1 1]
[0 1]
[0 0]].transpoe() tem erro na posição [0][1].
transpoe() não passou no(s) teste(s) acima
transpoe(): avaliação encerrada... (nota 0)

rearranja(): iniciando avaliação dos testes (vale 1 ponto(s))
rearranja() passou em todos os testes :-)
rearranja(): avaliação encerrada... (nota 1.0)

espelha(): iniciando avaliação dos testes (vale 1 ponto(s))
NumPymagem[[ 1 -1]
[ 1 1]].espelha(h) tem erro na posição [0][0].
NumPymagem[[ 1 -1]
[ 1 1]].espelha(v) tem erro na posição [0][1].
NumPymagem[[1 0 0]
[0 1 1]
[1 1 1]].espelha(h) tem erro na posição [0][0].
NumPymagem[[ 0 0 0]
[ 0 -1 0]].espelha(v) tem erro na posição [0][1].
NumPymagem[[ 1. 1. 1. 1. 1. 1. ]
[-1. -1. -1. 0.5 0.5 0.5]].espelha(v) tem erro na posição [0][0].
NumPymagem[[1 0]
[1 1]
[0 1]
[0 0]].espelha(h) tem erro na posição [0][0].
NumPymagem[[1 0]
[1 1]
[0 1]
[0 0]].espelha(v) tem erro na posição [0][0].
espelha() não passou no(s) teste(s) acima
espelha(): avaliação encerrada... (nota 0)
